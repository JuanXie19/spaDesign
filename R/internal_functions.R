############## These functions are from M Mukhopadhyay et al., J R Stat Soc Series B Stat Methodol. 2020
############# this is the main function for Fisher-Gaussian kernel
#####################
##### Libraries #####
#####################
library(MASS)
...
# x and mu are vector of same dimension, and tau is a scalar
#' @keywords internal
#' @noRd
dvmf <- function(x, mu, tau){   
    D <- length(mu)
...
#' @param: x:data; 
# K:#spheres; M:#vMF karnels; Iter:#iterations; tau_def:initial value of tau;
FG_mixture <- function(x, K, M, sigma_hat, Iter, tau_def, alpha, mm) {
  
  # Set parameters
  sig_frac <- 0 #sig_frac: a proportion of iterations below which we do not update sigma;  
  samp_IS <- 15   #samp_IS: minimum # samples required in a class to update tau in independent sampler;
  Iter2 <- 2000 # Iter2: #iterations for MH update of tau; sigma_hat:the maximum permissible value of sigma;
  ...
  # 1. sigma
  sig1 <- 1; sig2 <- 0.001
  sigma.sq <- invgamma::rinvgamma(1, shape = sig1, rate = sig2)
  sigma.sq <- sigma_1  
  # 2. Weights 
  if(missing(K)){K <- 30}
  if(missing(M)){M <- 5}
  ...
  for(i in 1:n) {
    l_use <- which(W[i,] == 1) 
    x_pr_sp <- Y[i,]
    weights <- sapply(1:M, function(m) {return(Pi[l_use,m]*dvmf(x_pr_sp,mu=mu[,l_use,m],tau=tau[l_use,m]))})
    if(sum(weights) == 0) {
      Z[i,] <- rep(0, ncol(Z))
...
#    print("update Pi")
    ...
    if(iter>(sig_frac*Iter)) {
      ...
      sigma.sq=invgamma::rinvgamma(1,shape=sig.shape,rate=sig.rate)
    ...
    for(l in 1:K) {
      ...
      # update r
      if(l_idx>0) { #rd[l]=mean(sapply(idx,function(u){return(norm_vec(x[u,]-cntr[,l]))})) }
